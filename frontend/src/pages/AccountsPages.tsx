...
interface AccountView {
  id: number
  origin: 'PURCHASE' | 'VENTA' | 'SALDO_INICIAL'
...
function AccountsPage({ mode }: { mode: AccountMode }) {
  ...
  const canCreateInitial = !isPayable && hasAnyAuthority('CXC_SALDOS_CREAR')
  ...
  const labels = isPayable
    ...
    : { eyebrow: 'Cobranza de clientes', title: 'Cuentas por cobrar', subtitle: 'Supervisa ventas al crédito, saldos históricos, vencimientos y cobranzas.', party: 'Cliente', origin: 'Venta', icon: 'bi-wallet2' }

  return <>
    <section className="accounts-page">
      <header className="page-header accounts-page__header"><div><span className="eyebrow">{labels.eyebrow}</span><h1>{labels.title}</h1><p>{labels.subtitle}</p></div><div className="page-header__actions">{canCreateInitial && <button className="primary-button primary-button--inline" type="button" onClick={() => setShowInitialBalance(true)}><i className="bi bi-journal-plus" /> Nuevo saldo inicial</button>}<button className="secondary-button secondary-button--inline" type="button" onClick={refresh} disabled={isLoading}><i className={`bi bi-arrow-clockwise ${isLoading ? 'inventory-spin' : ''}`} /> Actualizar</button></div></header>
...
function InitialBalanceModal({ clients, onClose, onSaved }: { clients: PartyOption[]; onClose: () => void; onSaved: () => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!clientId) next.idCliente = 'Selecciona el cliente responsable de la deuda.'
    if (!balance || Number(balance) <= 0) next.saldo = 'Ingresa un saldo mayor que cero.'
    if (!originDate) next.fechaOrigen = 'Indica la fecha de origen.'
    if (originDate > new Date().toISOString().slice(0, 10)) next.fechaOrigen = 'La fecha de origen no puede ser futura.'
    if (Object.keys(next).length) { setFieldErrors(next); return }
    const request: SaldoInicialRequest = { idCliente: Number(clientId), saldo: Number(balance), fechaOrigen: originDate, fechaVencimiento: dueDate || null, documentoReferencia: reference.trim() || null, observacion: observation.trim() || null }
    setIsSubmitting(true); setError(''); setFieldErrors({})
    try { await createInitialReceivable(request); onSaved() }
  ...
  return <div className="modal-backdrop account-payment-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && !isSubmitting && onClose()}><section className="account-payment-modal account-initial-balance-modal" role="dialog" aria-modal="true"><header><div><span><i className="bi bi-journal-plus" /></span><span><small>Cuenta histórica</small><h2>Registrar saldo inicial</h2></span></div><button className="icon-button" type="button" onClick={onClose} disabled={isSubmitting}><i className="bi bi-x-lg" /></button></header><form onSubmit={submit} noValidate><div>{error && <div className="alert-message alert-message--danger"><i className="bi bi-exclamation-circle-fill" /><span>{error}</span></div>}<div className="account-initial-notice"><i className="bi bi-shield-check" /><span><strong>Solo registra la deuda pendiente</strong><small>No genera venta, comprobante, movimiento de inventario ni ingreso en caja.</small></span></div><label><span>Cliente *</span><select value={clientId} onChange={(event) => setClientId(event.target.value)}><option value="">Seleccionar cliente</option>{clients.map((client) => <option key={client.id} value={client.id}>{client.label} · {client.document}</option>)}</select>{fieldErrors.idCliente && <small>{fieldErrors.idCliente}</small>}</label><label><span>Saldo pendiente *</span><div className="money-input"><span>S/</span><input type="number" min="0.01" step="0.01" value={balance} onChange={(event) => setBalance(event.target.value)} placeholder="0.00" /></div>{fieldErrors.saldo && <small>{fieldErrors.saldo}</small>}</label><div className="account-initial-date-grid"><label><span>Fecha de origen *</span><input type="date" max={new Date().toISOString().slice(0, 10)} value={originDate} onChange={(event) => setOriginDate(event.target.value)} />{fieldErrors.fechaOrigen && <small>{fieldErrors.fechaOrigen}</small>}</label><label><span>Fecha de vencimiento</span><input type="date" value={dueDate} onChange={(event) => setDueDate(event.target.value)} /></label></div><label><span>Documento o referencia</span><input value={reference} onChange={(event) => setReference(event.target.value)} maxLength={120} placeholder="Ej. Cuaderno 2026, boleta anterior" /></label><label><span>Observación</span><textarea value={observation} onChange={(event) => setObservation(event.target.value)} maxLength={500} rows={3} placeholder="Explica brevemente el origen del saldo" /></label></div><footer><button className="secondary-button" type="button" onClick={onClose} disabled={isSubmitting}>Cancelar</button><button className="primary-button primary-button--inline" type="submit" disabled={isSubmitting}>{isSubmitting ? <span className="spinner-border spinner-border-sm" /> : <i className="bi bi-check2" />} Registrar saldo</button></footer></form></section></div>
}
...
function normalizeAccount(item: CuentaPagar | CuentaCobrar, mode: AccountMode): AccountView {
  if (mode === 'payable') {
    ...
    return { id: account.id, origin: 'PURCHASE', originId: account.idCompra, partyId: account.idProveedor, party: account.proveedorRazonSocial, partyDocument: account.proveedorRuc, operationDate: `${account.fechaCompra}T00:00:00`, documentType: account.tipoComprobante || 'Documento interno', documentNumber: account.numeroComprobante || `Compra #${account.idCompra}`, condition: account.condicionPago, reference: null, observation: null, createdBy: null, total: account.total, paid: account.importePagado, balance: account.saldoPendiente, dueDate: account.fechaVencimiento, state: account.estado }
  }
  const account = item as CuentaCobrar
  const manual = account.origen === 'SALDO_INICIAL'
  return { id: account.id, origin: account.origen, originId: account.idVenta, partyId: account.idCliente, party: account.cliente || 'Cliente ocasional', partyDocument: account.clienteDocumento || 'Sin documento', operationDate: account.fechaVenta || `${account.fechaOrigen}T00:00:00`, documentType: manual ? 'Saldo inicial' : account.tipoComprobante || 'Documento interno', documentNumber: manual ? account.documentoReferencia || 'Sin referencia' : account.numeroComprobante || `Venta #${account.idVenta}`, condition: manual ? 'Saldo histórico' : account.condicionPago || 'Crédito', reference: account.documentoReferencia, observation: account.observacion, createdBy: account.usuarioCreacion, total: account.total, paid: account.importePagado, balance: account.saldoPendiente, dueDate: account.fechaVencimiento, state: account.estado }
}
function originLabel(account: AccountView, fallback: string) { return account.origin === 'SALDO_INICIAL' ? `Saldo inicial · @${account.createdBy}` : `${fallback} #${account.originId}` }
