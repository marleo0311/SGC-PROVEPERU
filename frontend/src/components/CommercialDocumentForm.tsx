...
export function CommercialDocumentForm({ kind, quote, onClose, onSaved }: Props) {
  ...
  const estimatedSubtotal = useMemo(() => lines.reduce((sum, line) => sum + Math.max(0, (Number(line.cantidad) || 0) * (line.precios[line.tipoPrecio] ?? 0) - (Number(line.descuento) || 0)), 0), [lines])
  const estimatedIgv = includeIgv ? Math.round(estimatedSubtotal * 0.18 * 100) / 100 : 0
